...
tg.turnOnMIDI = function () {
    ...
    var setupMIDI = () => {
        tg.midi = new OMGMIDI()
        tg.midi.onnoteoff = tg.onmidinoteoff;
        tg.midi.onnoteon = tg.onmidinoteon;
        tg.midi.onmessage = tg.onmidimessage;
        tg.midi.onplay = tg.onmidiplay;
        tg.midi.onstop = tg.onmidistop;

    }


    if (typeof OMGMIDI === "undefined") {
        omg.util.loadScripts(["/apps/music/js/webmidi.js"], setupMIDI)
    }
    else {
        setupMIDI()
    }
...
if (!tg.player.disableAudio) {
    moreScripts.push({url: "/apps/music/js/libs/peakmeter.js", onload: ()=> tg.peakMeters.toggle("All")})
}
